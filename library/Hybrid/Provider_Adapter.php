...
class Hybrid_Provider_Adapter {
	...
	function factory($id, $params = array()) {
		...
		if (isset($this->config["wrapper"]) && is_array($this->config["wrapper"])) {
			...
			// Get Provider Path.
			$provider_path = Hybrid_Auth::$config["path_providers"] . $this->id . ".php";
			
			if ( file_exists( $provider_path ) ) {
				require_once $provider_path;
			} else {
				do_action( 'logy_provider_file', $this->id );
			}

			$this->wrapper = "Hybrid_Providers_" . $this->id;

			// If Class Not Exist Go to error page.
			if ( ! class_exists( $this->wrapper ) ) {
				$redirect_url = add_query_arg( 'login', 'file_not_found', logy_page_url( 'login' ) );
